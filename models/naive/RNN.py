...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.wy = nn.Linear(input_size+hidden_size,output_size)
    ...
    def forward(self, input, hidden):
        ...
        #############################################################################
        T = input.shape[0]
        output = torch.zeros(size = (T,self.output_size))
        for t in range(T):
            cat = torch.cat((input[t],hidden[t]),0)
            hidden[t] = self.tanh(self.wh(cat))
